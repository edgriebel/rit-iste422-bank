...
def read_owners(filename: str) -> Sequence[Sequence[str]]:
    with open(filename) as f:
        recs = f.readlines()
    return [list(map(str.strip, r.split(','))) for r in recs][1:]

def save_accts(checking, savings) -> None:
    with open(PATH+'checking.csv', 'w') as cf, open(PATH+'savings.csv', 'w') as sf:
        cf.write('id,name,balance,checkNumber,ownerId,version\n')
        sf.write('id,name,balance,interestRate,ownerId,version\n')
        for c in checking:
...
def get_txns(acct: Sequence, is_checking: bool) -> Tuple[float, Sequence[Sequence[Any]]]:
    amounts = []
    total, start = 0.0, 0.0
    acct_id = acct[0]
    for _ in range(random.randint(5,30)):
        ...
        if total < 0:
            diff = 0 - total
            print(f'Adjusting starting bal: amt {amt:0.2f} tot={total:0.2f} diff={diff:0.2f} start={start:0.2f} + {diff:0.2f} = {start+diff:0.2f}')
            total = 0
            start += diff
...
def do_it():
    ...
    # print(owners)
    owners = [o.split(',') for o in owners]
...
def gen_dob():
    dob = datetime.datetime(random.randint(1940,2010), random.randint(1,12), random.randint(1,28))
...
def create_owners() -> Sequence[str]:
    owners = ['12346, Hilda Schrader Whitcher, 490075200750, 078-05-1120, 33 Main St, , Lockport, NY, 14094, v1']
    ...
    for i in range(10):
        ...
        owners.append(f'{owner_id}, {name}, {dob}, {ssn}, {addr[0]}, , {addr[1]}, {addr[2]}, {addr[3]}, v1')
        print(owners[-1])
    return owners
